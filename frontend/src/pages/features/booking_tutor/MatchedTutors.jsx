...
const TutorCard = ({ tutor, subjectName, isBestChoice }) => {
  ...
  return (
    <div
      className={`relative flex flex-col gap-4 bg-white rounded-2xl shadow-xl p-6 mb-6 ${
        isBestChoice ? "border-4 border-yellow-500" : ""
    ...
    >
      ...
      <div className="flex justify-between items-start">
        <div className="flex flex-wrap gap-2 max-w-[70%]">
          {teachingStylesArray.map((style, i) => (
            <span
              key={i}
              className="rounded-full px-2 py-1 bg-yellow-100 text-yellow-700 text-sm font-semibold border border-yellow-700"
            >
              {style}
            </span>
          ))}
        ...
        <div className="flex-shrink-0 self-start flex gap-5 text-yellow-600">
          <button
            className="underline text-sm hover:text-yellow-700"
          ...
          <button
            className="underline text-sm hover:text-yellow-700"
            onClick={() =>
              navigate(`/meetings/tutor/${tutor.userId}/${studentId}`)
            }
          >
            Calendar
          </button>
...
const MatchedTutors = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center bg-yellow-50 min-h-screen py-1 pb-40">
      <h1 className="text-2xl font-bold text-center mb-5 text-yellow-600">
        Top Matched Tutors
      </h1>
