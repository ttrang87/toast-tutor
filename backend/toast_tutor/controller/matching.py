...
from ..models import Course, Exam, TutorProfile, TutorRequest, User
from .algorithm import TutorMatcher


@api_view(["POST"])
def find_class_tutors(request):
    # create new request
    req = request.data
    ...
    try:
        TutorRequest.objects.create(user=user, description=description)
        ...
        try:
            if request_type == "subject":
                ...
                matching_course = Course.objects.filter(
                    user_id=tutor.id, name=subject_name, grade=grade
                ).first()
